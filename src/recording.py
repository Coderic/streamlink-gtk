    ...
    *ffmpeg_fout* must be a valid FFmpeg muxer for ``--ffmpeg-fout`` (e.g. ``matroska``, ``mp4``,
    ...
    return [
        *streamlink_invocation(),
        '--ffmpeg-fout',
        ffmpeg_fout,
        # Without a TTY, Streamlink may omit the progress line unless forced.
        '--progress',
        'force',
        # No interactive stdin: without this, Streamlink waits for [y/N] if the file exists.
        '--force',
        page_url,
        quality,
        '-o',
...
def launch_record_process(argv: Sequence[str]) -> subprocess.Popen:
    ...
    argv_list = list(argv)
    env = os.environ.copy()
    # Streamlink is Python: without this, stderr is often block-buffered when piped → empty UI log.
